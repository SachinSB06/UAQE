...
interface AccuracySizeScatterChartProps {
...
export const AccuracySizeScatterChart: React.FC<AccuracySizeScatterChartProps> = ({
  candidates,
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-semibold text-slate-900 tracking-tight">
                Top-1 Accuracy vs Model Footprint (MB)
              </h3>
              <SourceBadge source="MEASURED" origin="Host Validator" />
            </div>
            <p className="text-xs text-slate-500">
              Scatter plot of compression ratio against accuracy retention
            </p>
          </div>
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
            ...
            <XAxis
              ...
              fontSize={11}
              tickLine={false}
              label={{ value: 'Model Footprint (MB)', position: 'insideBottom', offset: -10, fontSize: 10, fill: '#94a3b8' }}
            />
            <YAxis
              type="number"
              dataKey="accuracy"
...
export default AccuracySizeScatterChart;
