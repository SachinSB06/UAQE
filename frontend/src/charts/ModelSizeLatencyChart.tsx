...
interface ModelSizeLatencyChartProps {
...
export const ModelSizeLatencyChart: React.FC<ModelSizeLatencyChartProps> = ({
  candidates,
  ...
  const points = [
    {
      id: 'fp32_baseline',
      name: 'FP32 Baseline',
      sizeMb: parseFloat(baselineSizeMB.toFixed(2)),
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-semibold text-slate-900 tracking-tight">
                Model Footprint (MB) vs Host CPU Latency (ms)
              </h3>
              <SourceBadge source="MEASURED" origin="Host Benchmark" />
            </div>
            <p className="text-xs text-slate-500">
              Compression vs Execution Latency Scatter
            </p>
          </div>
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
            ...
            <XAxis
              ...
              fontSize={11}
              tickLine={false}
              label={{ value: 'Model Size (MB)', position: 'insideBottom', offset: -10, fontSize: 10, fill: '#94a3b8' }}
            />
            <YAxis
              type="number"
              dataKey="latencyMs"
              ...
              fontSize={11}
              tickLine={false}
              label={{ value: 'Latency (ms)', angle: -90, position: 'insideLeft', offset: 15, fontSize: 10, fill: '#94a3b8' }}
            />
            <Tooltip
...
export default ModelSizeLatencyChart;
